function [ZVAR0,REF] = modneuberV4( ...
                                   ...
                                   Outfile)
% Implentation einer Modifizierten Neuber Methode f�r mehrachsige
% Kerbn�herungsverfahren.
%
% Idee, kombiniere Neuber mit Nebenbedingung f�r Umfangsspannung aus 
%       Hoffmannverfahren 
%
%  !!!!!!!  Implementierung f�r explizite Integration an normalen Punkten 
...
% REF0      -> Startwerte der Referenzzust�nde
...
if ntens ~= 3 && ndi ~= 2
    msg = 'Verfahren akt. nur f�r ESZ gedacht';
    error(msg)
end

% -------------------------------------------------------------------------
% Vorbereiten In-&Outputdateien
Infile = fopen(esigpath,'r');                                              % �ffne Inputdatei 
DATA0 = fread(Infile,[ntens+1,1],'double');                                % Startwerte
fout = fopen(Outfile,'w');                                                 % �ffne Ergebnissdatei
BSize = 10000;                                                             % Buffer

% -------------------------------------------------------------------------
% Definitionen f�r newton-verfahren
...
M = [2,-1,0;-1,2,0;0,0,3]./3;                                              % Abbildungen f�r Flie�fl�che
...
while aktdat <= numink
    ...
    % Setzten Startwerte
    DATA = [DATA0,DATA];
    % Ermittle Pseudo Elastische Gr��em
    ESIG = DATA(2:4,:);
    EEPS = DEL * ESIG;
    ...
    if sum(REF(:,1)) == 0 && aktdat == 2
        ...
        if jj == 1 % Erstes Inkrement plastisch -> Fehlermeldung
            ...
            %            ' ein elastisches Inkrement einf�gen'];
            %     error(msg)
    ...
    for ii = jj + 1 : space + 1
        ...
        if sum(ukp) == 0 && sum(eck) == 0
            ...
            % berechne tats�chliches Energieinkrement an ii-1
            ...
            [ZVAR(:,ii), dPHI] = newton_bisektion(...
                ...
                'ModNeuber',...                  % Definiert N�herungsverfahren
